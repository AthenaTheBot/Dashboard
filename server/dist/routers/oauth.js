...
const exchangeToken = (token) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const serverRes = yield (0, axios_1.default)({
            url: index_1.config.oauthEndpoints.tokenExchange,
            method: "POST",
            headers: {
                "Content-Type": "application/x-www-form-urlencoded",
            },
            data: `grant_type=authorization_code&client_id=${index_1.config.auth.clientId}&client_id=${index_1.config.auth.clientId}&client_secret=${index_1.config.auth.clientSecret}&redirect_uri=${index_1.config.auth.redirectUri}&code=${token}`,
        });
...
router.get("/login", (req, res) => {
    const parsedLink = index_1.config.links.login
